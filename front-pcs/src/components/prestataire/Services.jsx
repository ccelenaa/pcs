    ...
    <div className="tab-container">
      ...
      </div>
      {
        services.map((service) => {
          ...
          return (<>
            <div className="row">
              <div className="cell slim"><input type='checkbox' defaultChecked={service.prestataire !== undefined}/></div>
              <div className="cell">{service.label}</div>
              <div className="cell slim90" style={{textAlign: "right"}}>{service.prix_standard} €</div>
              <div className="cell slim">{service.prestataire ? (service.prix_standard > service.prestataire.prix ? '>' : '<'): ''}</div>
              <div className="cell slim80" style={{visibility: service.enabled === false || service.prestataire == undefined ? "hidden" : "visible"}}><input onChange={changePrix} data-serviceid={service.id} type='text' defaultValue={service.prestataire ? service.prestataire.prix : service.prix_standard} style={{textAlign: "right", fontWeight: 'bold', width: "50px", padding: "5px 5px",border: "1px solid rgba(50,50,50,0.2)", borderRadius: "3px"}}/> €</div>
              <div className="cell slim120" style={{display: 'flex'}}>
                <div style={{flexGrow: 1, textAlign: "left"}}>
                  {service.pcs_marge} %
                </div>
                <div style={{flexGrow: 1, textAlign: "right"}}>
                  {Math.round(service.pcs_marge * (service.prestataire ? service.prestataire.prix : service.prix_standard) / 100)} €
                </div>
              </div>
              <div className="cell slim90" style={{textAlign: "right", fontWeight: 'bold'}}>{Math.round((service.pcs_marge * (service.prestataire ? service.prestataire.prix : service.prix_standard) / 100) + (service.prestataire ? service.prestataire.prix : service.prix_standard))} €</div>
            </div>
          </>)
        })
      }
